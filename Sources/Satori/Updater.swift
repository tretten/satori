...
final class Updater: ObservableObject {
    ...
    /// Where the file lives. SATORI_FEED, for a test run, points somewhere
    ...
    static let feed: URL = {
        if let set = ProcessInfo.processInfo.environment["SATORI_FEED"], let url = URL(string: set) {
            return url
        }
        return URL(string: "https://github.com/tretten/satori/releases/latest/download/appcast.json")!
    }()

    private static var overridden: Bool {
        ProcessInfo.processInfo.environment["SATORI_FEED"] != nil
    }
    ...
    private func landed(_ release: Release, worked: Bool) {
        ...
        say?(worked
            ? "Satori \(release.version) is ready — it's there the next time you open it"
            : "Satori \(release.version) is out — it's in Settings")
    }
    ...
    private static var reopen: [String] {
        ...
        for key in ["SATORI_PROBE", "SATORI_FEED"] {
...
private enum Swap {
    ...
    static func install(_ release: Updater.Release) async throws {
        ...
        )) ?? files.temporaryDirectory.appendingPathComponent("satori-update-\(release.build)", isDirectory: true)
