...
enum ExtensionShims {
        ...
        if var background = manifest["background"] as? [String: Any] {
            if let worker = background["service_worker"] as? String {
                ...
                if var source = try? String(contentsOf: path, encoding: .utf8) {
                    ...
                    // Satori puts its newer shim in its place.
    ...
    (() => {
      ...
      // one — Safari's, as Satori's tabs send — not the Chrome one the
      ...
      const refuse = (what) => (...args) => {
        const callback = args.length && typeof args[args.length - 1] === "function" ? args.pop() : null;
        const error = new Error(what + " isn't available in Satori");
      ...
      // shown in Satori, so there is nothing for them to do.
      ...
      // The popup Satori shows is a page of its own, known to WebKit as a
      ...
      // Satori opens a popup itself (see Extensions.press), and has to know
      ...
      // rest still go in: one rule Satori can't honour shouldn't cost an
      ...
      // Context menu entries for places Satori has no menu for — the old
      ...
      // no group — Satori has none), which code tests before anything else;
      ...
      // where Chrome answers false. The ones Satori answers itself are
      // Satori's to grant: those a manifest names are granted, optional
      // ones are asked for.
    ...
    static func answer(_ message: Any, from context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        guard let body = message as? [String: Any], let api = body["api"] as? String else {
            return ["error": "Not a Satori message"]
        }
    ...
    private static func run(_ api: String, _ args: [Any], context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        ...
        case "downloads.pause", "downloads.resume", "downloads.cancel", "downloads.removeFile", "downloads.getFileIcon":
            throw Unsupported(what: "\(api) isn't available in Satori yet")
        ...
        // MARK: permissions Satori grants itself
        ...
        case "power.requestKeepAwake":
            ...
            if IOPMAssertionCreateWithName(kind, IOPMAssertionLevel(kIOPMAssertionLevelOn), "An extension in Satori" as CFString, &assertion) == kIOReturnSuccess {
                awake[id] = assertion
            }
            return nil
        ...
        case "power.reportActivity":
            var assertion: IOPMAssertionID = 0
            IOPMAssertionDeclareUserActivity("An extension in Satori" as CFString, kIOPMUserActiveLocal, &assertion)
            return nil
        ...
        case "readingList.addEntry", "readingList.removeEntry", "readingList.updateEntry":
            throw Unsupported(what: "Satori has no reading list")
        ...
        case "tabGroups.get", "tabGroups.update", "tabGroups.move":
            throw Unsupported(what: "Satori has no tab groups")
        ...
        default:
            throw Unsupported(what: "\(api) isn't available in Satori")
        }
    }
    ...
    /// launches as Chrome keeps it. Satori acts on one of them — an
    ...
    /// chrome.browsingData, from what WebKit and Satori keep.
