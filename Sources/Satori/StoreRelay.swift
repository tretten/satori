...
// "Switch to Chrome", and an "Add to Chrome" button that stays grey. Satori
...
// "Add to Satori" one — the button people already look for, rather than a bar
...
final class StoreRelay: NSObject, WKScriptMessageHandler {
    ...
    (function () {
      ...
      function render(ours) {
        ...
        label(ours, installed ? 'Added to Satori' : (busy ? 'Adding…' : 'Add to Satori'));
